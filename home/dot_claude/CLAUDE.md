...
Never edit files or commit unless explicitly instructed to. Never ask whether the user wants a commit either.

Always ask clarification questions.

`rg` (ripgrep) and `jq` are installed. Use `rg` to search for text and `jq` to read JSON when either would be more efficient than the alternative.
